...
export function FAQCard({ jobId }: Props) {
  ...
  } = useQuery<QuestionData>(QUESTIONS_BY_JOB, {
    variables: { job: jobId },
  });
  ...
  return (
    <S.Wrapper>
      ...
      {questionsLoading ? (
        ...
        <div>Erro ao carregar perguntas</div>
      ) : (
        questions!.user_job_questions.map(
          ({
            id,
            question,
            user_job_question_replies: answers,
            user: { name: userName },
          }) => (
            <>
              <S.QuestionHeader>
                ...
                <S.QuestionHeaderText>
                  <S.QuestionHeaderName>{userName}</S.QuestionHeaderName>
